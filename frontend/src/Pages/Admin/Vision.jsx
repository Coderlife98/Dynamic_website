...
const Vision = () => {
  ...
  const handleVision = async (event) => {
    ...
    try {
      ...
      const addedVision = await axios.post(
        `${Base_url}vision/Vision/add`,
        formData,
      );
      if (addedVision) {
        toast.success(addedVision.data.message);
        setHeading("");
        setImage(null);
        setStatus("");
        setDescription("");
  ...
  const getVision = async () => {
    try {
      const getData = await axios.get(`${Base_url}vision/get`);
      if (getData) {
  ...
  return (
    <div>
      ...
      <div className="mt-4  md:py-4   px-2 border border-slate-800  text-white">
        ...
        <form ref={formRef} onSubmit={handleVision} className="md:px-8">
