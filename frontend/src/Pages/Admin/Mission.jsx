...
const Mission = () => {
  ...
  const handleMission = async (event) => {
    ...
    try {
      ...
      toast.error(operation.data.message);
    }
  };

  const getData = async () => {
    try {
      const get = await axios.get(`${Base_url}mission/get`);
      if (get) {
  ...
  return (
    <div>
      ...
      <div className="mt-4  md:py-4   px-2 border border-slate-800  text-white">
        ...
        <form ref={formRef} onSubmit={handleMission} className="md:px-8">
